...
spa.chat = (function () {
    ...
    var configMap = {
        ...
        setJqueryMap, getEmSize, setPxSizes, setSliderPosition, onClikToggle,
    ...
    // 目的：初期化前にモジュールを構成する
    // 引数：
    //   * set_chat_anchor - オープンクローズ状態を示すようにURIアンカーを変更するコールバック
    ...
    //     完全なリストはconfigMap.settable_map
    //     用例：slider_open_emはem単位のオープン時の高さ
    // 動作：
    //   指定された引数で内部構成データ構造(configMap)を更新する
    //   その他の動作は行わない
    // 戻り値：true
    // 例外発行：受け入れられない引数や欠如した引数ではjavascriptオブジェクトとスタックトレースをスロー
    //
    ...
    initModule = function ( $append_target ) {
        ...
        jqueryMap.$head.click(onClikToggle);
    ...
    // 目的：
    //   ウインドウリサイズイベントに対し必要に応じてこのモジュールが提供する表示を調整
    // 目的：
    //   ウインドウの高さや幅が所定の閾値を下回ったら、
